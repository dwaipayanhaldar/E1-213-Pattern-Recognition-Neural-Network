...
class RandomForest():
    ...
    def build_tree(self, X, y, depth=0, available_features=None):
        ...
        if available_features is None:
            available_features = list(range(X.shape[1]))  # shared mutable list

        majority_class = int(np.bincount(y, minlength=2).argmax())

        # Leaf conditions: max depth, pure node, no samples, or no features left
