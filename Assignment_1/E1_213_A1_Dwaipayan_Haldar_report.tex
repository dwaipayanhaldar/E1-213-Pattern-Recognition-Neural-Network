...
\begin{solution}
The target dataset is threshold by median to give a binary dataset. The data is not linearly separable. So hard margin SVM will not be able to fit in the data. I used \verb|scipy.optimize.minimize| to optimize, it is not able to converge and going on some infinite loop. Using \verb|cvxopt.solvers.qp| was able to solve the problem but all the mu was very large suggesting that all of them are support vectors which is clearly not the case. All of these occurs as the optimization problem is not feasible. 
...
\begin{solution}
Fig. \ref{fig:3_7a}, Fig. \ref{fig:3_7b} and Fig. \ref{fig:3_7c} gives the plots of Log-Likelihood vs Iteration for different values of K = 2,4 and 6. When the covariance matrix becomes singular, causing \verb|numpy.linalg.LinAlgError| during inversion in the multivariate normal PD. 
\end{solution}
\begin{figure}[H]
\centering
...
\begin{solution}
Index of point exactly on the margin 3374 and the value of mu for this is 0.0004879. Index of point classified safely outside the margin 8564 and the value of mu for this is $7.87 \times 10^{-13}$. No point has a $\mu$ close to 1. Highest $\mu$ is in the range pf $10^{-4}$ So, we can safely conclude that there is no point inside the margin. 

\end{solution}
\textbf{3.10 Mercer's Condition}

\begin{solution}
The Kernel matrix satisfy the Mercer's condition. The highest and the lowest eigenvalue is near 1 for small $\sigma$ and when the $\sigma$ becomes high, the matrix is ill conditioned but the lowest eigen value is always greater than 0. So, the matrix is PSD. 
\end{solution}
...
\textbf{4.13 Vectorization Benchmarking}
\begin{solution}
The total time of running the Naive KNN is 25.78 secs, while the vectorized KNN takes only 0.53 secs. That is almost $50\times$ faster. 
\end{solution}

\textbf{4.14 Curse of Scale}
\begin{solution}
The accuracy without the standard scalar is coming out to be 88.85\% while the accuracy with the standard scalar is coming out to be 91.65\%. Intuitively, that makes sense the unscaled data should give lower accuracy.  
\end{solution}

\textbf{4.15 Bayes optimal \& Underflow}
\begin{solution}
The Naive Bayes classifier with log conditional is implemented on the whole dataset and it gives an accuracy of 91.31\%. The underflow occurs at D=28 with \verb|float32|.
\end{solution}

\questionheader{Phase 5: Bias-Variance Decomposition}

\textbf{5.16 Empirical Bias-Variance via Bootstrapping}

\begin{solution}
The bias of the $d = 1$ model and the $d = 15$ model are 2.33 and 2.31 respectively. While the variance is 0.001 and 0.01 respectively. This is expected the variance of the $d = 15$ model is higher while the bias is relatively less than $d=1$ model. That is expected since the former model tries to fit in perfectly. 
\end{solution}

\textbf{5.17 Frequentist vs. Bayesian Uncertainity}

\begin{solution}
The frequentist variance is 0.0011 and the analytical bayesian variance is 0.0014. Fig. \ref{fig:5_17} gives the plot of both the distributions. \\
\textbf{Frequentist:} Expectation over repeated datasets $\mathcal{D}$:
\[
\mathbb{E}_{\mathcal{D}}\!\left[\hat{\theta}\right], \qquad \mathrm{Var}_{\mathcal{D}}\!\left[\hat{\theta}\right] = \mathbb{E}_{\mathcal{D}}\!\left[\left(\hat{\theta} - \mathbb{E}[\hat{\theta}]\right)^2\right]
\]
\textbf{Bayesian:} Expectation over the posterior $p(\theta \mid \mathcal{D})$:
\[
\mathbb{E}_{\theta \mid \mathcal{D}}\!\left[\theta\right] = \int \theta\, p(\theta \mid \mathcal{D})\, d\theta, \qquad \Sigma_{\text{post}} = \left(\frac{X^\top X}{\sigma^2} + \frac{I}{\tau^2}\right)^{-1}
\]
\end{solution}

\begin{figure}[H]
\centering
\includegraphics[scale = 0.5]{P05_17.png}
\caption{Distribution Function calculated analytically and with the frequentist approach}
\label{fig:5_17}
\end{figure}
\end{document}